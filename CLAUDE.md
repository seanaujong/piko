...
| What the cursor is on is the note holding the sentence under it, never that sentence — one answer, asked by the hover and by the click that follows it | ✅ | `content/extraction/sentences.ts` (`noteCovering`) | `sentences.test.ts`, `highlight.browser.test.ts`, `e2e/extension.test.ts` (`drops a whole grown note`) |
| A reach lands on a note by joining it; the note it reaches *from* is the nearest one it is not already part of | ✅ | `content/extraction/sentences.ts` (`passageExtendedTo`) | `sentences.test.ts`, `e2e/extension.test.ts` (`joins two notes`) |
| A note the reach only partly covers is taken whole — superseding one the span does not reach the end of would delete the rest of it | ✅ | `content/extraction/sentences.ts` (`passageExtendedTo`) | `sentences.test.ts` |
...
| A table is the article's one child that cannot wrap: it scrolls itself rather than widening the column, and never stretches to fill it | ✅ | `content/panel/styles.ts` (`.piko-article table`) | `extractedView.browser.test.ts` |
